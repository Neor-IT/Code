...
def send_welcome(message):
    bot.reply_to(message, "Привіт! Відправ спочатку зображення, а потім текст і я зроблю з цього QR-Code!")
...
def handle_docs_photo(message):
    ...
    try:
        ...
        path_to_download = Path().joinpath(src)  # Шлях до фону з QR-Code
        bot.reply_to(message, "Фото отримано! Пришліть текст!")
...
def send_text_based_qr(message):
    ...
    try:
        ...
        bot.reply_to(message, 'Ваш текст прийнято!\nОчікуйте.')
