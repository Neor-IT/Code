# Voice Assistant 0.8
"""
    Ассистент разработан на версии python 3.9
    OS: Windows 10
"""
import pyttsx3
...
class Assistant:
    ...
    def weather(self):
        ...
        owm = OWM('API KEY')  # ключ с сайта open weather map
        ...
        self.talk(f"В городе {place} сейчас {status}"  # Выводим город и статус погоды в нём
                  f"\nТемпература {round(temp)} градусов по цельсию"  # Выводим температуру с округлением в ближайшую сторону
                  f"\nВлажность составляет {humidity}%"  # Выводим влажность в виде строки
                  f"\nСкорость ветра {w.wind()['speed']} метров в секунду")  # Узнаём и выводим скорость ветра
